...
class Checkpoint:
    ...
    def multistep(self) -> int:
        return self._metadata.training.multistep
    ...
    def graph(self) -> HeteroData:
        ...
        """
        return self._model_instance.graph_data if hasattr(self._model_instance, "graph_data") else None
    ...
    def update_graph(self, path: Optional[str] = None) -> HeteroData:
        ...
        if self.UPDATE_GRAPH:
            raise RuntimeError("Graph has already been updated. Mutliple updates is not allowed")
        else:
            if path:
    ...
    def name_to_index(self) -> dict:
        ...
        if isinstance(_data_indices, (tuple, list)) and len(_data_indices)>=2:
            return {f"grid{k}" : _data_indices[k].name_to_index for k in range(len(_data_indices))}
            
        return self._model_instance.data_indices.name_to_index #{name : index for index,name in enumerate(self._metadata.dataset.variables)}
    ...
    def index_to_name(self) -> dict:
        ...
        _data_indices = self._model_instance.data_indices
        if isinstance(_data_indices, (tuple, list)) and len(_data_indices)>=2:
            return {k : {index: var for var, index in self.name_to_index[k].items()} for k in self.name_to_index}
    ...
    def model_output_index_to_name(self) -> dict:
        """
            ...
            return:
                ...
                data.output.full
        """
        if isinstance(self._metadata.data_indices, (tuple, list)) and len(self._metadata.data_indices)>= 2:
            mapping = {
    ...
    def model_output_name_to_index(self) -> dict:
        ...
        """
        if isinstance(self._metadata.data_indices, (tuple, list)) and len(self._metadata.data_indices)>= 2:
            return {k : {name : index for index, name in v.items() } for k, v in self.model_output_index_to_name.items()}
